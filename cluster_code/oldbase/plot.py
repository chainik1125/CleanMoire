...
# directory = os.fsencode("/pathdata_cluster")
# dstr="/pathdata_cluster"
...
def file_names(directory,contains,kpoints,variable,parameterv,theta):#omegas
    ...
    for file in os.listdir(directory):
        ...
        print(f'file in folder: {filename}')
...
def chained_path_plot(path_lists,kpoints,directory,dstr,mu_shift,params,variable,theta):
    ...
    if np.isclose(np.abs(w1),0)==False:
        ...
        w1string='0'
    fig.suptitle(r'One particle $\theta=$'+f'{angle}'+r'$^{\circ}$'+',  '+r'$U_{HK}=$'+params.split('K')[-1]+', '+r'$v_{f}=$'+f'{v}'+r'$eV A^{-1}$'+', '+r'$|K|=$'+f'{Kmag}'+', '+r'$w_1$='+f'{w1}'+r'$eV$'+', '+r'$w_0$='+f'{w1string}'+r'$w_1$'+f' shells={shells_used}', fontsize=16)
...
if __name__ == "__main__":
    ...
    directory = os.fsencode("/Users/dmitrymanning-coe/Documents/Research/Barry Bradlyn/Moire/Numerics/MoireCluster/pathdata5")
    dstr="/Users/dmitrymanning-coe/Documents/Research/Barry Bradlyn/Moire/Numerics/MoireCluster/pathdata5"
    # directory = os.fsencode("/pathdata2")
    # dstr="/pathdata2"    
    mu_shift1=0
    params=f'UHK{UHK}UHKrot{UHK_rot}m'#UHKtau{UHK_N_p0t0pxp0}m'
    chained_path_plot(path_lists=[[A,B,C,D,A]],kpoints="20",directory=directory,dstr=dstr,mu_shift=mu_shift1,params=params,variable=f'{particle_no}particle',theta=f'theta{thetadeg}') 
