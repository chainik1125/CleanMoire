...
def diagonalization_scheme(ham_k,start_sparse=True,howmany=16):
...
def chained_path_plot_link(path_list,kpoints,generate_Hk,UHK,mu,Umu,Utau,Uff,names_reversed_var):
    ...
    for i in range(0,len(path_list)-1):
        ...
        for j in x_values:
            ham_k=generate_Hk(kx=j[0],ky=j[1],particles_used=particle_no)
            twoparticleenergies=diagonalization_scheme(ham_k)[0]
    ...
    progress_bar.close()
    dirname=pathdata_folder_cluster+f"/mu{mu}UHK{UHK}UHKrot{UHK_rot}Utau{Utau}kp{kpoints}theta{round(thetadeg,2)}"
