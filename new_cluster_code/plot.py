...
def chained_path_plot(path_lists,kpoints,directory,dstr,mu_shift,params,variable,theta):
    ...
    if np.isclose(np.abs(w1),0)==False:
        ...
        w1string='0'
    fig.suptitle(r'One particle $\theta=$'+f'{angle}'+r'$^{\circ}$'+',  '+r'$U_{HK}=$'f'{UHK}'+', '+r'$U_{HK,rot}=$'f'{UHK_rot}'+', '+r'$v_{f}=$'+f'{v}'+r'$eV A^{-1}$'+', '+r'$|K|=$'+f'{Kmag}'+', '+r'$w_1$='+f'{w1}'+r'$eV$'+', '+r'$w_0$='+f'{w1string}'+r'$w_1$'+f' shells={shells_used}', fontsize=16)
...
if __name__ == "__main__":
    ...
    mu_shift1=0
    params=f'UHK{1}m'
    chained_path_plot(path_lists=[[A,B,C,D,A]],kpoints="20",directory=directory,dstr=dstr,mu_shift=mu_shift1,params=params,variable=f'{4}particle',theta=f'theta{thetadeg}') 
