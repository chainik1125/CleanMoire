...
from oldbase.variables_cluster2 import *
...
directory = os.fsencode("pathdata5")
dstr="pathdata5"
...
def chained_path_plot(path_lists,kpoints,directory,dstr,mu_shift,params,variable,theta,linesplotted,plotcutoff):
    ...
    if np.isclose(np.abs(w1),0)==False:
        ...
        w1string='0'
    fig.suptitle(r'Four particle $\theta=$'+f'{angle}'+r'$^{\circ}$'+',  '+r'$U_{HK}=$'+params.split('K')[-1]+', '+r'$v_{f}=$'+f'{v}'+r'$eV A^{-1}$'+', '+r'$|K|=$'+f'{Kmag}'+', '+r'$w_1$='+f'{w1}'+r'$eV$'+', '+r'$w_0$='+f'{w1string}'+r'$w_1$'+f' shells={shells_used}', fontsize=16)
    
    #plt.show()
    dirname=f'pathdata5/graphs/graphs_{particle_no}particle_theta{thetadeg}_UHK{UHK}_UHKrot{UHK_rot}_UHKtau{UHK_N_p0t0pxp0}'
    os.makedirs(dirname,exist_ok=True)
    
    plt.savefig(dirname+f'/first{linesplotted}',dpi=500,bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    mu_shift1=0
    params=f'UHK{UHK}UHKrot{UHK_rot}UHK_tau{UHK_N_p0t0pxp0}m'
    chained_path_plot(path_lists=[[A,B,C,D,A]],kpoints="10",directory=directory,dstr=dstr,mu_shift=mu_shift1,params=params,variable=f'{4}particle',theta=f'theta{thetadeg}',linesplotted=16,plotcutoff=16) 
